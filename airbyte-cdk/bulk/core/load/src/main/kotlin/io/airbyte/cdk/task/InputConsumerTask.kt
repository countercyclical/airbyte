...
class InputConsumerTask(
    ...
    private val inputFlow: Flow<Reserved<String>>,
    ...
    override suspend fun execute() {
        ...
        try {
            checkpointQueue.use {
                inputFlow.collect { reserved ->
                    val message = deserializer.deserialize(reserved.value)
                    val sizeBytes = reserved.value.length.toLong()
                    when (message) {
...
@Singleton
class ReservingInputStreamFlow(
    private val config: DestinationConfiguration,
    private val memoryManager: MemoryManager,
    private val inputStream: InputStream,
) : Flow<Reserved<String>> {
    val log = KotlinLogging.logger {}

    override suspend fun collect(collector: FlowCollector<Reserved<String>>) {
        ...
        reservation.use { _ ->
            inputStream.bufferedReader().lineSequence().forEachIndexed { index, line ->
                ...
                val estimatedSize = line.length.toLong() * config.estimatedRecordMemoryOverheadRatio
                val reserved = reservationManager.reserveBlocking(estimatedSize.toLong(), line)
                collector.emit(reserved)
